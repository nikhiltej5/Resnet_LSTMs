...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shape0 = out.shape[2]
        shape1 = out.shape[3]
        out = LayerNorm([shape0,shape1])(out)
        out = self.relu(out)
        out = self.conv2(out)
        shape0 = out.shape[2]
        shape1 = out.shape[3]
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=25):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 16,256, layers[0])
        self.layer2 = self._make_layer(block, 32, 128,layers[1])
        self.layer3 = self._make_layer(block, 64, 64,layers[2])
        self.avgpool = nn.AdaptiveAvgPool2d((1,1))
        self.fc = nn.Linear(64, num_classes)

    def _make_layer(self, block, planes,shape0, blocks, stride=1):
        downsample = None
        if self.inplanes != planes:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes, planes, kernel_size=3, stride=2,padding=1),
                LayerNorm([shape0,shape0])
            )
