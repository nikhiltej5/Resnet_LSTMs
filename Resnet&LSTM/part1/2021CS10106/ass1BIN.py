...
class BatchNorm(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            with torch.no_grad():
                running_temp = (1 - self.momentum) * self.running_mean
                running_temp = running_temp.to(device)
                self.running_mean = self.momentum * mean + running_temp
                # update running_var with unbiased var
                running_vartemp = (1 - self.momentum) * self.running_var
                running_vartemp = running_vartemp.to(device)
                self.running_var = self.momentum * var * n / (n - 1) + running_vartemp
        else:
...
class InstanceNorm(nn.Module):
    def __init__(self, num_features, momentum=0.1, eps=1e-5):
        ...
        self.eps = eps
        self.num_features = num_features
        self.running_mean = torch.zeros((32,self.num_features))
        self.running_var = torch.ones((32,self.num_features))
        self.weight = nn.Parameter(torch.ones(self.num_features)) ## TODO
        self.bias = nn.Parameter(torch.zeros(self.num_features)) ## TODO

    def forward(self, input):
        
        inputsize0 = input.shape[0]
        inputsize2 = input.shape[2]
        
        input = input.view(32,self.num_features,inputsize2,-1)
        ...
        input = input * self.weight[None, :, None, None] + self.bias[None, :, None, None]
        input = input.view(inputsize0,self.num_features,inputsize2,-1)
        return input
...
class BatchInstanceNorm(nn.Module):
    def __init__(self,num_features):
        ...
        self.batch = BatchNorm(self.inplanes)
        self.instance = InstanceNorm(self.inplanes)
    def forward(self,input):
...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        ...
        self.batch1 = BatchInstanceNorm(out_channels)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.batch2 = BatchInstanceNorm(out_channels)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_data_file', type=str, help='Path to test data')
    parser.add_argument('--model_file', type=str, help='Path to model file')
    parser.add_argument('--output_file', type=str, help='Path to output file')
    parser.add_argument('--n', type=int, help='Number of layers')
